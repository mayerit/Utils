...
WHERE  ( A.FLG_SITUACAOREGISTRO = 1 
	-- DEFAULT COM COD_SISTEMA, COD_FUNC E NOM_FORMULARIO
	--AND A.COD_SISTEMA = 210)
	--AND A.COD_FUNC = @CODFUNC )
	--AND (RTRIM(LTRIM(UPPER(B.NOM_FORMULARIO))) = N'FRMEQUIPFECHADURA' )

	--SIGA AND A.COD_SISTEMA = 210) AND (RTRIM(LTRIM(UPPER(B.NOM_FORMULARIO))) = N'FRMEQUIPFECHADURA')
	--SICOM AND A.COD_SISTEMA = 146) AND (RTRIM(LTRIM(UPPER(B.NOM_FORMULARIO))) = N'FRMEQUIPFECHADURA')
	AND A.COD_SISTEMA = 146) AND (RTRIM(LTRIM(UPPER(B.NOM_FORMULARIO))) = N'FRMTRIBUTACAO')

--BUSCA OBJETOS POR SUBSTRING
SELECT SM.OBJECT_ID, OBJECT_NAME(SM.OBJECT_ID) AS OBJECT_NAME, O.TYPE, O.TYPE_DESC, SM.DEFINITION  
FROM SYS.SQL_MODULES AS SM  
JOIN SYS.OBJECTS AS O ON SM.OBJECT_ID = O.OBJECT_ID  
WHERE UPPER(SM.DEFINITION) LIKE '%EXTENSO%'
ORDER BY O.TYPE;
...
FROM SYS.SQL_MODULES AS SM  
JOIN SYS.OBJECTS AS O ON SM.OBJECT_ID = O.OBJECT_ID  
WHERE OBJECT_NAME(SM.OBJECT_ID) = UPPER('%SP_SIS_SEL_CORBOLSA%')
ORDER BY O.TYPE;
...
SELECT 'SYSDATETIME()      ', CONVERT (date, SYSDATETIME()), SYSDATETIME();
SELECT 'SYSDATETIMEOFFSET()', CONVERT (date, SYSDATETIMEOFFSET()), SYSDATETIMEOFFSET();  
SELECT 'SYSUTCDATETIME()   ', CONVERT (date, SYSUTCDATETIME()), SYSUTCDATETIME();  
